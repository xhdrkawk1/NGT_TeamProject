...
{
	Arrow1_Interval = 1.f;// 2초당 하나
	Arrow1_Speed = 150.f; // 1초에 50이동
	Arrow2_Interval = 4.f;//4초에 하나
	Arrow2_Speed = 110.f; // 1초에 50이동
...
void CObjectMgr::Stage1()
{
	...
	{
		PLAYERDATA* pdata = CDataMgr::GetInstance()->m_tPlayerData;

		if (pdata[0].Alive && pdata[1].Alive)
			Target = Target % 2;
		else if (pdata[0].Alive && pdata[1].Alive == false)
			Target = 0;
		else  if (pdata[0].Alive == false && pdata[1].Alive)
			Target = 1;
		else
		{
			Target = Target2 % 2;
	...
	if (fcount4 >= Stage4_Interval)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			for (int j = 0; j < 8; j++)
			{
				...
				OBJECT_ARROW *ArrowInfo = new OBJECT_ARROW;
				ArrowInfo->Pos = Pos[j];
				ArrowInfo->Dir = Player_Pos[idx] - ArrowInfo->Pos;
				D3DXVec3Normalize(&ArrowInfo->Dir, &ArrowInfo->Dir);   //크기를 정규화
				D3DXMatrixTranslation(&ArrowInfo->mat_Trans, ArrowInfo->Pos.x, ArrowInfo->Pos.y, 0.f);  //점들을 좌표로
				D3DXVECTOR3 vStandard2 = -ArrowInfo->Dir;
				float cter = D3DXVec3Dot(&vStandard2, &vStandard); //cos cter  화살이 가지는 방향벡터와 -x축 방향벡터를 내적함 (라디안)
				...
				if (ArrowInfo->Pos.y <= Player_Pos[idx].y)
				{
					D3DXMatrixScaling(&ArrowInfo->mat_Scale, 1.f, 1.f, 0.f);
					D3DXMatrixRotationZ(&ArrowInfo->mat_Rotation, (Angle));
				}
...
{
	Rect->left = LONG(POS.x - (32* 0.5));
	Rect->top =  LONG(POS.y - (10* 0.2));
	Rect->right =LONG(POS.x + (32* 0.4));
	Rect->bottom=LONG(POS.y + (10 *0.2));
}
...
void CObjectMgr::Clear_AllObject()
{
	...
	for (; iterBeginWarning != iterEndWarning; )
	{
		...
		Safe_Delete(*iterBeginWarning);//레이저 X 생성
